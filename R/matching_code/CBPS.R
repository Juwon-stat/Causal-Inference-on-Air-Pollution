...
make_exact_key <- function(dt, vars) {
  do.call(
    paste,
    c(lapply(dt[, ..vars], as.character), sep = "||")
  )
}
...
extract_cbps_ps <- function(fit, n) {
  ...
  for (nm in cand) {
    if (!is.null(fit[[nm]])) {
      p <- suppressWarnings(as.numeric(fit[[nm]]))
...
run_cbps_one <- function(DT, q, region_var, d) {
  ...
  for (td in dates) {
    ...
    pool[, exact_key := make_exact_key(.SD, exact_covs)]
    ...
    if (!length(common_keys)) {
      drop_list[[length(drop_list) + 1L]] <- data.table(block_date = td, reason = "no_common_exact_support")
      next
    }
    ...
    if (!nrow(pool) || length(unique(pool$treated)) < 2) {
      drop_list[[length(drop_list) + 1L]] <- data.table(block_date = td, reason = "insufficient_after_exact_support")
      next
    }

    ps_covs <- if (cfg$USE_REDUCED_CBPS_MODEL) setdiff(covs, exact_covs) else covs
    ...
    norm <- pool[
      ,
      .(
        Wt = sum(raw_weight[treated == 1L], na.rm = TRUE),
        Wc = sum(raw_weight[treated == 0L], na.rm = TRUE),
        nt = sum(treated == 1L),
        nc = sum(treated == 0L)
      ),
      by = exact_key
    ...
    pool <- merge(pool, norm, by = "exact_key", all.x = TRUE)
    pool <- pool[is.finite(Wt) & Wt > 0 & is.finite(Wc) & Wc > 0]

    if (!nrow(pool) || length(unique(pool$treated)) < 2) {
      drop_list[[length(drop_list) + 1L]] <- data.table(block_date = td, reason = "invalid_exact_cell_weights")
      next
    }
    ...
    if (!nrow(pool) || length(unique(pool$treated)) < 2) {
      drop_list[[length(drop_list) + 1L]] <- data.table(block_date = td, reason = "empty_weighted_rows")
      next
    }
